...
void main() {
  ...
  test('sends only the scene values that resolve to a real MIDI parameter', () async {
    ...
    final knobControlId = await controlRepository(database).createControl(
      ampId,
      const ControlDraft(name: 'Amp Knob 1', type: ControlType.percentage, minValue: 0, maxValue: 100),
    );
    final unrelatedControlId = await controlRepository(database).createControl(
      ampId,
      const ControlDraft(
    ...
    await scenePedalRepository(database).addPedal(sceneId: sceneId, pedalId: ampId);
