...
Future<PresetImportDecision> _alwaysOverwrite(
  ImportedPreset incoming,
  Patch existing,
) async => PresetImportDecision.overwrite;

Future<PresetImportDecision> _alwaysSkip(
  ImportedPreset incoming,
  Patch existing,
) async => PresetImportDecision.skip;
...
void main() {
  ...
  test(
    'imports a preset into a patch, scene and matching scene values',
    () async {
      const preset = ImportedPreset(
        ...
        blocks: [
          ImportedBlock(
            label: 'Amp',
            modelNumber: 12,
            parameters: {'Amp Knob 1': 70},
          ),
        ],
      );
      final service = presetImportService(
        database,
        const _FakeTransferService([preset]),
      );
      ...
      final scenes = await sceneRepository(
        database,
      ).watchScenes(numbered.single.patch.id).first;
      expect(scenes.single.name, 'Imported');

      final blocks = await pedalRepository(
        database,
      ).watchComponentPedals(unitId).first;
      final ampBlock = blocks.firstWhere((pedal) => pedal.name == 'Amp');
      final scenePedals = await scenePedalRepository(
        database,
      ).watchScenePedals(scenes.single.id).first;
      expect(scenePedals.map((pedal) => pedal.id), contains(ampBlock.id));

      final controls = await controlRepository(
        database,
      ).watchControls(ampBlock.id).first;
      final modelControl = controls.firstWhere(
        (control) => control.name == 'Amp model',
      );
      final knobControl = controls.firstWhere(
        (control) => control.name == 'Amp Knob 1',
      );
      final values = await sceneValueRepository(
        database,
      ).watchValues(scenes.single.id).first;
      expect(
  ...
  test(
    'silently skips a block that is not one of this profile\'s seeded pedals',
    () async {
      const preset = ImportedPreset(
        programNumber: 1,
        name: 'Odd Preset',
        confidence: MidiSupportLevel.unknown,
        blocks: [
          ImportedBlock(label: 'Not A Real Block', parameters: {'X': 1}),
        ],
      );
      final service = presetImportService(
        database,
  ...
  test(
    'a duplicate program number, skipped, leaves the existing patch untouched',
    () async {
      const first = ImportedPreset(
        programNumber: 1,
        name: 'First',
        confidence: MidiSupportLevel.unknown,
      );
      final service = presetImportService(
        database,
        const _FakeTransferService([first]),
      );
      await service.importPresets(
        unitId: unitId,
        presets: [first],
        resolveDuplicate: _alwaysSkip,
      );

      const again = ImportedPreset(
        programNumber: 1,
        name: 'Replacement',
        confidence: MidiSupportLevel.unknown,
      );
  ...
  test(
    'a duplicate program number, overwritten, replaces the existing patch',
    () async {
      const first = ImportedPreset(
        programNumber: 1,
        name: 'First',
  ...
  test(
    'a preset that fails to import is recorded as a failure, not thrown',
    () async {
      const first = ImportedPreset(
        programNumber: 1,
        name: 'Same Name',
      ...
      final service = presetImportService(
        database,
        const _FakeTransferService([first, second]),
      );
  ...
  test(
    'countOverwrites is 0 and nothing changes before importPresets runs',
    () async {
      const first = ImportedPreset(
        programNumber: 1,
        name: 'Existing',
  ...
  test(
    'fetchPresets reports progress as MockNuxMg30V5PresetTransferService reads each preset',
    () async {
      final service = presetImportService(
        database,
      ...
      final presets = await service.fetchPresets(
        onProgress: (completed, total) => progressTicks.add(completed),
      );
