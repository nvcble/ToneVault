...
void main() {
  test('identifyRequest matches the reference project\'s exact bytes', () {
    expect(NuxMg30V5SysEx.identifyRequest().toBytes(), [0xF0, 0x43, 0x58, 0x00, 0xF7]);
  });

  test('getCurrentProgramNumberRequest matches the reference project\'s exact bytes', () {
    expect(
      NuxMg30V5SysEx.getCurrentProgramNumberRequest().toBytes(),
      [0xF0, 0x43, 0x58, 0x70, 0x15, 0x00, 0xF7],
    );
  });

  test('getPresetDataRequest fills in the program number and pads to 15 bytes', () {
    expect(
      NuxMg30V5SysEx.getPresetDataRequest(21).toBytes(),
      [0xF0, 0x43, 0x58, 0x70, 0x0B, 0x00, 21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF7],
    );
  });
  ...
  test('isPresetDataResponse requires both the exact header and a known dump length', () {
  ...
  test('isPresetDataResponse is false for a same-header response of the wrong length', () {
    const shortResponse = SysExMessage(payload: [0x43, 0x58, 0x70, 0x0B, 0x02]);

    expect(NuxMg30V5SysEx.isPresetDataResponse(shortResponse), isFalse);
  });

  test('isIdentifyResponse reads the ASCII firmware string out of the reply', () {
