...
void main() {
  ...
  test('decodes per-scene bypass for two blocks sharing the same scene bytes', () {
    final message = _fixture((bytes) {
    ...
    final compressor = decoded.blocks.firstWhere((b) => b.label == 'Compressor');
  ...
  test('Delay is parallel if either the MOD/DLY bit or the DLY/RVB bit is set', () {
    final modDlyOnly = decodeNuxMg30V5Preset(_fixture((bytes) => bytes[146] = 2));
    final dlyRvbOnly = decodeNuxMg30V5Preset(_fixture((bytes) => bytes[146] = 4));
  ...
  group('a real 222-byte dump off the physical MG-30 V5', () {
    ...
    test('reports the v4.0.3-only fields as unsupported instead of guessing', () {
