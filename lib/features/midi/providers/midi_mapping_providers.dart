...
final Provider<MidiPatchSelectionSettingsDao> midiPatchSelectionSettingsDaoProvider =
    Provider<MidiPatchSelectionSettingsDao>(
      (ref) => MidiPatchSelectionSettingsDao(ref.watch(appDatabaseProvider)),
    );

final Provider<MidiParameterMappingRepository> midiParameterMappingRepositoryProvider =
    Provider<MidiParameterMappingRepository>(
      (ref) => MidiParameterMappingRepository(ref.watch(midiParameterOverrideDaoProvider)),
    );

final Provider<PatchSelectionRepository> patchSelectionRepositoryProvider =
    Provider<PatchSelectionRepository>(
      (ref) => PatchSelectionRepository(ref.watch(midiPatchSelectionSettingsDaoProvider)),
    );

/// One device profile's parameters, with any user remapping already applied.
final StreamProviderFamily<List<MidiParameterDefinition>, String> effectiveParametersProvider =
    StreamProvider.family<List<MidiParameterDefinition>, String>((ref, deviceProfileId) {
      final profile = MidiDeviceRegistry.findById(deviceProfileId);
      if (profile == null) {
        return const Stream<List<MidiParameterDefinition>>.empty();
      }
      return ref.watch(midiParameterMappingRepositoryProvider).watchEffectiveParameters(profile);
    });

/// The names of one device profile's parameters that have been remapped away
/// from their defaults.
final StreamProviderFamily<Set<String>, String> overriddenParameterNamesProvider =
    StreamProvider.family<Set<String>, String>(
...
final StreamProviderFamily<PatchSelectionOverride?, String> patchSelectionOverrideProvider =
    StreamProvider.family<PatchSelectionOverride?, String>(
      (ref, deviceProfileId) =>
          ref.watch(patchSelectionRepositoryProvider).watchOverride(deviceProfileId),
    );
